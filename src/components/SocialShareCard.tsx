  ...
  return (
    <section className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
      <div className="score-card p-6">
        ...
        <h2 className="mt-3 text-3xl font-black text-ink">猜中啦，发一张战报卡</h2>
        <p className="mt-3 leading-7 text-ink/60">
          每次用户猜中、登上周榜、击败某个 AI 选手，都可以生成一张 1080x1350 的竖版海报，用于朋友圈、小红书、X 或 Instagram。
        </p>
        <div className="mt-5 flex flex-wrap gap-3">
          <button className="angled-button inline-flex items-center gap-2 bg-fifaBlue px-5 py-3 font-black text-white">
            <Share2 className="h-4 w-4" /> 分享战报
          </button>
      ...
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-fifaBlue via-fifaPurple to-neonBlue p-6 text-white shadow-glow">
        ...
        <div className="relative">
          ...
          <h3 className="mt-8 text-5xl font-black leading-none">猜中啦!</h3>
          <p className="mt-3 text-white/70">北看台阿岚命中「法国 vs 韩国最终胜者」</p>
          <div className="mt-8 rounded-xl bg-white p-5 text-ink">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-bold text-ink/50">本题得分</p>
                <p className="text-4xl font-black text-fifaBlue">+4 PTS</p>
              </div>
            </div>
            <div className="mt-5 grid grid-cols-3 gap-3 text-center">
              <Mini label="总排名" value="#12" />
              <Mini label="击败 AI" value="GPT" />
              <Mini label="连续命中" value="3" />
            </div>
          </div>
          <p className="mt-5 text-sm font-bold text-white/70">扫码加入竞猜，和 AI 大模型同场 PK</p>
        </div>
      </div>
    </section>
  );
