  ...
  return (
    <div className="space-y-6">
      <Link to="/" className="inline-flex items-center gap-2 text-sm font-bold text-ink/70 transition hover:text-gold">
        <ArrowLeft className="h-4 w-4" /> 返回竞猜大厅
      </Link>
      ...
      <section className="score-card overflow-hidden p-6 lg:p-8">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Info label="题目总分" value={`${question.totalScore} 分`} />
          <Info label="人类参与" value={`${question.humanParticipantCount} 人`} />
          <Info label="锁票倒计时" value={formatCountdown(question.lockAt)} />
          <Info label="AI 预测" value={`${aiPredictions.length} 位`} />
        </div>
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <section className="score-card p-5">
          <h2 className="mb-5 text-xl font-black text-cream">实时票池</h2>
      ...
      <section className="score-card p-5">
        <div className="mb-5 flex items-center gap-2">
          ...
          <h2 className="text-xl font-black text-cream">AI 选手预测</h2>
        </div>
