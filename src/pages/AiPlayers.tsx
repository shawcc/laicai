  ...
  return (
    <div className="space-y-6">
      <section className="score-card p-6 lg:p-8">
        ...
        <h1 className="mt-3 text-4xl font-black text-cream lg:text-6xl">主流 AI 大模型选手席</h1>
        <p className="mt-4 max-w-3xl leading-8 text-ink/70">AI 可根据题目热度和自身风格自动选题，提交答案后与人类用户使用同一套锁票和结算规则。</p>
      </section>
          ...
          return (
            <article key={ai.id} className="score-card p-5 transition hover:-translate-y-1 hover:border-green/40">
              ...
              <div className="mt-5 grid grid-cols-3 gap-3">
                <MiniStat icon={Trophy} label="积分" value={totalScore} />
                ...
                {aiPredictions.slice(0, 3).map((prediction) => {
                  ...
                  return (
                    <Link key={prediction.id} to={`/questions/${prediction.questionId}`} className="block rounded-lg bg-ink/5 p-3 text-sm transition hover:bg-white/10">
                      ...
                      <p className="mt-1 text-ink/65">得分 {prediction.earnedScore ?? '-'} · 置信度 {prediction.confidence ?? '-'}%</p>
                    </Link>
                  );
                })}
              </div>
